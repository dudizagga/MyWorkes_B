...
public class Disconnect extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
        context = getActivity();
        View discon = inflater.inflate(R.layout.activity_disconnect, base, false);
        rs = discon.findViewById(R.id.recycler_d);
        lin = new LinearLayoutManager(context);
        u = new utlShared(context);
        rs.setHasFixedSize(true);
        rs.setLayoutManager(lin);
        Toast.makeText(context, "frag d", Toast.LENGTH_SHORT).show();
                ...
                Backendless.Persistence.of(Carmel.class).find(new AsyncCallback<List<Carmel>>() {
                    ...
                    public void handleResponse(List<Carmel> response) {
                        for (int i = 0; i < response.size(); i++) {
                            if (response.get(i).reason.equals("disconnect")) {
                                UsersDetails userd = new UsersDetails(response.get(i).user, response.get(i).lastName,
                                        response.get(i).clazz, response.get(i).roomNumber, response.get(i).phoneNumber,
                                        response.get(i).reason);
